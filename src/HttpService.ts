...
export default class HttpService {
  ...
  static getController<T>(host: string, params: IServiceParams) {
    return ServiceHelper.executeService<T>(host, {
  ...
  static GetIds(host: string) {
    return ServiceHelper.executeService<string[]>(host, {
  ...
  /** 执行控制器 */
  private static _executeController<T>(
    host: string,
    params: ISaveServiceParams
  ) {
    return ServiceHelper.executeService<T>(
      host,
      {
        name: "/service/controller/execute?_ajax=1",
        params
